...
2. If it is the first line, when a user pays another user, it will create node, edge, record how many neighbor(s) each node has and value of all degrees. The set of degree values then calculate to get the median.

3. The degree itself will be used for next line as comparison.

4. For the next line, when a user pays another user, it will check whether it falls in 60-second window or not. If it is in 60-second window, again, it will create node, edge, record how many neighbor(s) each node has and new value of all degrees by comparing with previous value of all degrees.
Before comparing, the code (the function you can see in <i>def entries</i>) will create a new empty value of degree list and do the comparison between current value of all degrees and previous value of all degrees by check each number in previous value of all degrees against each number in current value of degrees. If found, delete that number in the current value of degrees and that number will be added into new empty degrees list and if unable to found means it exists in previous value of degrees but not current value of degrees, it will add into new empty degree list and until all numbers in current value of degrees has been compared, if it exists in current value of degrees, it will add into new empty degree list. <b>Remember:</b> it will add numbers from previous value of degrees first before adding number(s) from current value of degrees. By doing so, we maintain the order of the degrees.
